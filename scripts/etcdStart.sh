...
EXE=etcd
COMMAND=$APP_DIR/deps/linux/etcd-server/$EXE
#CONFIG=$APP_DIR/configs/
PID_FILE=$APP_DIR/configs/etcd-server.pid
LOG_FILE=$APP_DIR/logs/etcd-server.log

help()
{
    echo ""
    echo "start script"
    echo "Usage: ./etcdStart.sh [-h]"
    echo ""
}
...
if [[ "$OS_TYPE" =~ "Darwin" ]];then
    brew services start etcd-server
else
    ## run command
